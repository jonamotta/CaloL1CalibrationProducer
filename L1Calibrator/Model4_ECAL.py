...
if __name__ == "__main__" :
    ...
    ## opening L1 file with jet trigger towers
    df = pd.read_csv('/data_CMS/cms/motta/CaloL1calibraton/2022_04_02_NtuplesV0/hdf5dataframes/test.csv')
    df = df[['jetId','i_eta','i_em','i_had','i_et']]
    df["i_esum"] = df["i_em"] + df["i_had"]
    df.set_index('jetId',inplace=True)
    df_e = df[['i_eta','i_esum']]

    ## opening gen file with jet pT
    dfjetPt = pd.read_csv('/data_CMS/cms/motta/CaloL1calibraton/2022_04_02_NtuplesV0/hdf5dataframes/testPt.csv')
    dfjetPt = dfjetPt[['jetId','jetPt']]
    dfjetPt.set_index('jetId', inplace=True)
