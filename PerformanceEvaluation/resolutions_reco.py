...
for i in range(0, nevents):
    ...
    for igenJet in range(0,reco_nJets):
        ...
        if matched:
            pt_response_ptInclusive.Fill(highestL1Pt/reco_jet.Pt())

            if abs(reco_jet.Eta()) < 1.305:
                pt_barrel_resp_ptInclusive.Fill(highestL1Pt/reco_jet.Pt())
            elif abs(reco_jet.Eta()) < 5.191 and abs(reco_jet.Eta()) > 1.479:
                pt_endcap_resp_ptInclusive.Fill(highestL1Pt/reco_jet.Pt())

            for i in range(len(ptBins)-1):
                if reco_jet.Pt() > ptBins[i] and reco_jet.Pt() <= ptBins[i+1]:
                    response_ptBins[i].Fill(highestL1Pt/reco_jet.Pt())
                    
                    if abs(reco_jet.Eta()) < 1.305:
                        barrel_response_ptBins[i].Fill(highestL1Pt/reco_jet.Pt())
                    elif abs(reco_jet.Eta()) < 5.191 and abs(reco_jet.Eta()) > 1.479:
                        endcap_response_ptBins[i].Fill(highestL1Pt/reco_jet.Pt())

            for i in range(len(etaBins)-1):
                if abs(reco_jet.Eta()) > etaBins[i] and abs(reco_jet.Eta()) < etaBins[i+1]:
                    absEta_response_ptBins[i].Fill(highestL1Pt/reco_jet.Pt())

                if reco_jet.Eta() > etaBins[i] and reco_jet.Eta() < etaBins[i+1]:
                    plusEta_response_ptBins[i].Fill(highestL1Pt/reco_jet.Pt())

                elif reco_jet.Eta() < -etaBins[i] and reco_jet.Eta() > -etaBins[i+1]:
                    minusEta_response_ptBins[i].Fill(highestL1Pt/reco_jet.Pt())

            k = 0
            for i in range(len(ptBins)-1):
                for j in range(len(etaBins)-1):
                    if abs(reco_jet.Eta()) > etaBins[j] and abs(reco_jet.Eta()) < etaBins[j+1] and reco_jet.Pt() > ptBins[i] and reco_jet.Pt() < ptBins[i+1]:
                        pt_resp_PtEtaBin[k].Fill(highestL1Pt/reco_jet.Pt());
