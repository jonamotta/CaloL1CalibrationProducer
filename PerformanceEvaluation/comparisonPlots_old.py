...
newcalib_label = "New Calibration"
# newcalib_label = "Calibraton"
...
if options.doResponse == True:
    ...
    for ibin in range(0,histo.GetNbinsX()):
        ...
        X_err.append(histo.GetBinWidth(ibin+1)/2.)
        Y_err.append(histo.GetBinError(ibin+1))
    ax.errorbar(X, Y, xerr=X_err, yerr=Y_err, label=newcalib_label, ls='None', lw=2, marker='o', color='green', zorder=2)
    ...
    for xtick in ax.xaxis.get_major_ticks():
        xtick.set_pad(10)
    plt.grid()
    if options.reco: mplhep.cms.label('Internal', data=True, rlabel='(13.6 TeV)')
...
if options.doResolution == True:
    ...
    ax.errorbar(X_s_newcalib, Y_s_newcalib, yerr=Y_r_newcalib, label=newcalib_label, ls='None', lw=2, marker='o', capsize=3, color='green', zorder=2, transform=trans_r)
    ...
    if options.do_HoTot:
        ...
        for ibin in range(0,histo.GetNbinsX()):
            ...
            X_err.append(histo.GetBinWidth(ibin+1)/2.)
            # Y_err.append(histo.GetBinError(ibin+1))
        ax.errorbar(X, Y, xerr=X_err, label=newcalib_label, ls='None', lw=2, marker='o', color='green', zorder=2)
        X_r_newcalib = X; Y_r_newcalib = Y
        Ymax = max(Ymax, max(Y))
...
if options.doTurnOn == True:
    ...
    for thr in options.thrsFixRate:
        ...
        plt.xlim(0, 60) if int(thr) < 30 else plt.xlim(0, 200)
        plt.ylim(0., 1.05)
        for xtick in ax.xaxis.get_major_ticks():
            xtick.set_pad(10)
        plt.grid()
