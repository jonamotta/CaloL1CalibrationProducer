...
file_unCalib  = ROOT.TFile("ROOTs/resolution_graphs_"+label+"_uncalib.root", "r")
file_oldCalib = ROOT.TFile("ROOTs/resolution_graphs_"+label+"_oldCalib.root", "r")
file_newCalib = ROOT.TFile("ROOTs/resolution_graphs_"+label+"_newCalib.root", "r")
...
inclusive_resp_unCalib.GetYaxis().SetRangeUser(0., max(inclusive_resp_oldCalib.GetMaximum(),inclusive_resp_newCalib.GetMaximum())*1.3 )
...
inclusive_resp_unCalib.Draw()
inclusive_resp_newCalib.Draw("same")
inclusive_resp_oldCalib.Draw("same")
...
for i in range(len(ptBins)-1):
    ...
    ptBins_resp_unCalib.Draw()
    ptBins_resp_newCalib.Draw("same")
    ptBins_resp_oldCalib.Draw("same")
...
for i in range(len(etaBins)-1):
    ...
    etaBins_resp_unCalib.Draw()
    etaBins_resp_newCalib.Draw("same")
    etaBins_resp_oldCalib.Draw("same")
...
ptBins_resol_unCalib.Draw()
ptBins_resol_newCalib.Draw("same")
ptBins_resol_oldCalib.Draw("same")
...
etaBins_resol_unCalib.GetYaxis().SetRangeUser(0., 2.)
...
etaBins_resol_unCalib.Draw()
etaBins_resol_newCalib.Draw("same")
etaBins_resol_oldCalib.Draw("same")
...
ptBins_scale_unCalib.Draw()
ptBins_scale_newCalib.Draw("same")
ptBins_scale_oldCalib.Draw("same")
...
etaBins_scale_unCalib.Draw()
etaBins_scale_newCalib.Draw("same")
etaBins_scale_oldCalib.Draw("same")
...
file_unCalib  = ROOT.TFile("ROOTs/rate_graphs_uncalib.root", "r")
file_oldCalib = ROOT.TFile("ROOTs/rate_graphs_oldCalib.root", "r")
file_newCalib = ROOT.TFile("ROOTs/rate_graphs_newCalib.root", "r")
...
rateDi_unCalib.Draw()
rateDi_newCalib.Draw("same")
rateDi_oldCalib.Draw("same")
...
for i in range(1,240):
    if rateDi_newCalib.GetBinContent(i) < oldCalibAt60:
        thrNewCalib = rateDi_newCalib.GetBinLowEdge(i-1)
        break

for i in range(1,240):
    if rateDi_unCalib.GetBinContent(i) < oldCalibAt60:
        thrUnCalib = rateDi_unCalib.GetBinLowEdge(i-1)
        break
...
texl1.AddText("")
texl1.AddText("Uncalibrated:      fixed "+str(round(oldCalibAt60,1))+" kHz rate #rightarrow DoubleJet"+str(round(thrUnCalib)))
texl1.AddText("New Calibration: fixed "+str(round(oldCalibAt60,1))+" kHz rate #rightarrow DoubleJet"+str(round(thrNewCalib)))
...
rate_unCalib.Draw()
rate_newCalib.Draw("same")
rate_oldCalib.Draw("same")
...
for i in range(1,240):
    if rate_newCalib.GetBinContent(i) < oldCalibAt60:
        thrNewCalibJet = rate_newCalib.GetBinLowEdge(i-1)
        break

for i in range(1,240):
    if rate_unCalib.GetBinContent(i) < oldCalibAt60:
        thrUnCalibJet = rate_unCalib.GetBinLowEdge(i-1)
        break
...
texl1.AddText("")
texl1.AddText("Uncalibrated:      fixed "+str(round(oldCalibAt60,1))+" kHz rate #rightarrow SingleJet"+str(int(round(thrUnCalibJet))))
texl1.AddText("New Calibration: fixed "+str(round(oldCalibAt60,1))+" kHz rate #rightarrow SingleJet"+str(int(round(thrNewCalibJet))))
texl1.SetTextSize(0.02)
texl1.SetFillColor(0)
texl1.SetBorderSize(0)
texl1.SetTextAlign(11)
...
for i in range(1,240):
    if rate_newCalib.GetBinContent(i) < oldCalibAt32:
        thrNewCalibEG = rate_newCalib.GetBinLowEdge(i-1)
        break

for i in range(1,240):
    if rate_unCalib.GetBinContent(i) < oldCalibAt32:
        thrUnCalibEG = rate_unCalib.GetBinLowEdge(i-1)
        break
...
texl1.AddText("")
texl1.AddText("Uncalibrated:      fixed "+str(round(oldCalibAt32,1))+" kHz rate #rightarrow SingleEG"+str(int(round(thrUnCalibEG)))+"er2p5")
texl1.AddText("New Calibration: fixed "+str(round(oldCalibAt32,1))+" kHz rate #rightarrow SingleEG"+str(int(round(thrNewCalibEG)))+"er2p5")
texl1.SetTextSize(0.02)
texl1.SetFillColor(0)
texl1.SetBorderSize(0)
texl1.SetTextAlign(11)
...
file_unCalib  = ROOT.TFile("ROOTs/efficiency_graphs_"+label+"_uncalib.root", "r")
file_oldCalib = ROOT.TFile("ROOTs/efficiency_graphs_"+label+"_oldCalib.root", "r")
file_newCalib = ROOT.TFile("ROOTs/efficiency_graphs_"+label+"_newCalib.root", "r")

if label == "HCAL":
    thrUnCalib = thrUnCalibJet
    thrNewCalib = thrNewCalibJet
    thrOldCalib = 60
if label == "ECAL":
    thrUnCalib = thrUnCalibEG
    thrNewCalib = thrNewCalibEG
    thrOldCalib = 32

turnon_unCalib  = file_unCalib.Get("divide_passing"+str(int(round(thrUnCalib)))+"_by_total")
turnon_oldCalib = file_oldCalib.Get("divide_passing"+str(int(round(thrOldCalib)))+"_by_total")
turnon_newCalib = file_newCalib.Get("divide_passing"+str(int(round(thrNewCalib)))+"_by_total")
...
turnon_unCalib.Draw()
turnon_newCalib.Draw("same")
turnon_oldCalib.Draw("same")

legend = ROOT.TLegend(0.55,0.75,0.88,0.88)
legend.SetBorderSize(0)
legend.AddEntry(turnon_unCalib,"Uncalibrated: p_{T}^{L1 jet}>"+str(int(round(thrUnCalib)))+" GeV","LPE")
legend.AddEntry(turnon_oldCalib,"Old Calibration: p_{T}^{L1 jet}>"+str(int(round(thrOldCalib)))+" GeV","LPE")
legend.AddEntry(turnon_newCalib,"New Calibration: p_{T}^{L1 jet}>"+str(int(round(thrNewCalib)))+" GeV","LPE")
legend.Draw("same")
...
if label == "HCAL":
    canvas.SaveAs("PDFs/comparisons_"+label+"/turnons_DoubleJetXX.pdf")
    canvas.SaveAs("PNGs/comparisons_"+label+"/turnons_DoubleJetXX.png")
if label == "ECAL":
    canvas.SaveAs("PDFs/comparisons_"+label+"/turnons_DoubleEGXXer2p5.pdf")
    canvas.SaveAs("PNGs/comparisons_"+label+"/turnons_DoubleEGXXer2p5.png")
