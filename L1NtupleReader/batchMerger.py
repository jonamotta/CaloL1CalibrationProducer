...
if __name__ == "__main__" :
    ...
    parser.add_option("--filesLim",               dest="filesLim",               default=100000, type=int,            help="Maximum number of npz files to use")
    ...
    parser.add_option("--ECALcalib4rate",         dest="ECALcalib4rate",         default=None,                        help="Model for ECAL calibration in HCAL rate ('/data_CMS/cms/motta/CaloL1calibraton/'+options.ECALcalib4rate+'/model_ECAL/TTP')")
    ...
    InFilesRate = glob.glob(filedir+'/'+options.ratedir+'/tensors/towers_*.npz')
    InFilesRateBlocks = splitInBlocks(InFilesRate, 100)
    ...
    with tf.device('/CPU:0'):
        ...
        for blockIdx, block in enumerate(InFilesTrainBlocks):
            ...
            X = X[(uncalibResp < 3) & (uncalibResp > 0.3)]
            Y = Y[(uncalibResp < 3) & (uncalibResp > 0.3)]
            del uncalibResp
